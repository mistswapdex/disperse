...
const Ether = ({ address }) => {
  ...
  const disperseEther = async () => {
    try {
      ...
      if (ethereum && disperseAddresses[chainId]) {
        ...
        console.log("Dispersing WDOGE now");
        console.log(total);
        ...
        console.log("Completed dispersing WDOGE");
      }
    } catch (error) {
      console.log("error occured while dispersing WDOGE");
      console.log(error);
    }
  };
  ...
  return (
    <p className="pt-4 text-l font-light italic">
      you have {ethBalance} <span className="pt-1 text-sm">WDOGE</span>
      <Recipients
        textValue={textValue}
